...
est4_boot <- nonprob(
  ...
  method_outcome = "glm",
  family_outcome = "binomial",
  control_inference = controlInf(var_method = "bootstrap", rep_type = "subbootstrap", num_boot = 50),
  verbose = T
)
```
...
wyniki <- rbind(
  ...
  cbind(est4_sel_bias$output,   est4_sel_bias$confidence_interval)
) |>
  transform(est = c("IPW", "PMM", "NN", "MI", "DR", "DR (SCAD)", "DR (SCAD, MIN)"))
